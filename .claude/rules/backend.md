...
- **Dívidas conhecidas congeladas (NÃO corrigir sem decisão):** `summary()` passa `"ALUNO"` String a `countByTipoAndAtivoTrue` (campo é enum `UserType`) → **500** (`MealEntitlementFlowIT#summaryQuebraPorBugDeTipo`); `DEVICE_DENIED` usado p/ subtipo desconhecido (falta `UNKNOWN_EVENT`); guard `isEmpty()` vs `isBlank()` no webhook (espaços → 500); `@PreAuthorize` sem token → 403 (não 401).
