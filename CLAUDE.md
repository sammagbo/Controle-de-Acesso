...
- **Testes automatizados:** `mvn test` → **507 testes, 0 falhas, 2 `@Disabled`** (medido em 07/08 na branch das câmeras da portaria; eram 183 no fim da Fase I). `npm test` → **58 testes, 0 falhas**. ⚠️ o `pom.xml` tem `maven-surefire-plugin` com `<include>*IT.java` — **sem isso o Surefire pula os ITs em silêncio**. As 2 queries nativas PostgreSQL-only ficam `@Disabled` (H2 não roda) → conferência manual obrigatória na V13.
