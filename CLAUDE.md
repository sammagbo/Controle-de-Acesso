...
- **Testes automatizados:** `mvn test` → **350 testes, 0 falhas, 2 `@Disabled`** (medido em 06/08 sobre `2902e74`; eram 183 no fim da Fase I). `npm test` → **58 testes, 0 falhas**. ⚠️ o `pom.xml` tem `maven-surefire-plugin` com `<include>*IT.java` — **sem isso o Surefire pula os ITs em silêncio**. As 2 queries nativas PostgreSQL-only ficam `@Disabled` (H2 não roda) → conferência manual obrigatória na V13.
