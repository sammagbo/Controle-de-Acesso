...
- **Bug corrigido (16/07, `f947373`):** o feed de negadas nunca exibia dado — `getRefectoryAttempts`/`getGateAttempts` existiam em `js/utils/api.js` (Fase H) mas **nunca eram anexadas ao `window.api`**; o `CantineMonitor` caía no fallback `[]` **em silêncio** (curl no endpoint devolvia a attempt; a tela dizia "Aucune tentative"). Ligado no fim de `api.js`. ⚠️ **Pendência aberta:** `GeneralReport.js:1112` renderiza `<DeniedAttemptsFeed/>` **sem props** (`fetchFn` undefined → caixa de erro) — decidir fonte (geral paginado `/api/access/attempts` vs refeitório) ou remover.
