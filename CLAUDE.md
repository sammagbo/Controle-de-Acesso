...
- **Testes automatizados:** `mvn test` → **603 testes, 0 falhas, 2 `@Disabled`** (medido em 10/08 na branch feat/gate-matching-and-small-fixes; eram 569 antes dela e 183 no fim da Fase I). `npm test` → **279 testes, 0 falhas**. ⚠️ o `pom.xml` tem `maven-surefire-plugin` com `<include>*IT.java` — **sem isso o Surefire pula os ITs em silêncio**. As 2 queries nativas PostgreSQL-only ficam `@Disabled` (H2 não roda) → conferência manual obrigatória (seção 6-bis do `docs/frontend-smoke-checklist.md`); a assimetria do filtro de posto fixo em `currentOccupancyByPoint` tem um guarda de STRING (`AccessLogRepositoryQueryGuardTest`) porque a suíte não executa a consulta.
