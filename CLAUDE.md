...
- **Testes automatizados:** `mvn test` → **546 testes, 0 falhas, 2 `@Disabled`** (medido em 10/08 na branch do posto fixo; eram 533 antes dela e 183 no fim da Fase I). `npm test` → **253 testes, 0 falhas**. ⚠️ o `pom.xml` tem `maven-surefire-plugin` com `<include>*IT.java` — **sem isso o Surefire pula os ITs em silêncio**. As 2 queries nativas PostgreSQL-only ficam `@Disabled` (H2 não roda) → conferência manual obrigatória na V13.
