...
- **Camada de decisão implementada (Fases A–K da `docs/architecture/ESPECIFICACAO-TECNICA-v1.md`).** Fases A–H commitadas (HEAD `ff41ed1`); Fases I (testes) e J (migrations) implementadas, ainda **não commitadas**. Validação com hardware: **smoke test 3/3 OK (16/07)**; bateria completa V01–V14 ainda **pendente** — só após `mvn test` verde.
...
- **Backlog pré-piloto (NÃO implementar sem decisão):** **F7a** = destaque **visual + som** para item novo no `DeniedAttemptsFeed` (o feed é peça crítica de UX do modelo assistido — ADR-004); **F7b** = botão de **exportação CSV no formato do HCP** (`Person ID` = `hikvision_employee_id` como TEXTO, zeros à esquerda; **nunca** passar pelo Excel) — só após o template do HCP ser definido (pendência com o Fabiano).
