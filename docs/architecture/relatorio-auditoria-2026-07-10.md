...
## 9. Riscos
**R1 [BLOQ p/ piloto]** Frontend depende de CDNs (React/Tailwind/Babel/lucide/jspdf) — kiosk offline não renderiza. Ação: vendorizar em `libs/`. **R2 [ALTO, confirmar]** String com padrão de senha (`Republique@1`) exposta como e-mail de autor em commits antigos e no `.mailmap` público. Se é/foi senha real em qualquer serviço: **rotacionar já**; avaliar filter-repo (decisão antiga do Sam, reaberta). **R3** IPs por DHCP (PC e terminal) — 3 trocas em 3 dias; pedir reservas ao SI. **R4** Defaults inseguros sem env: ADMIN_PIN=1234 (sem WARN de startup — melhoria M2), admin/admin1234, JWT dev. Cobertos parcialmente por WARNs. **R5** ddl-auto não relaxa constraints (precedente door_no). **R6** CORS `*`+credentials (aceitável interno; revisar na VM). **R7** Zero testes automatizados — regressões dependem de bench manual.
