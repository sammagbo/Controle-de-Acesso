...
function PortariaModal({ user, responsavel, onConfirm, onCancel }) {
    if (!user) return null;

    return (
        <div className="fixed inset-0 z-[100] bg-navy-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white/95 rounded-[32px] w-full max-w-4xl shadow-2xl overflow-hidden border border-white/20 animate-zoom-in">
                <div className="p-8 space-y-8">

                    {/* ALUNO ROW */}
                    <div className="flex items-center gap-8 bg-soft-50 p-6 rounded-3xl border border-soft-200">
                        <img src={user.foto_url} alt={user.nome} className="w-32 h-32 rounded-full shadow-md border-4 border-white flex-shrink-0" />
                        <div className="flex-1 space-y-3">
                            <div className="bg-white border border-soft-200 rounded-xl px-5 py-3 shadow-sm">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Nome do Aluno</p>
                                <h2 className="text-2xl font-black text-navy-500">{user.nome}</h2>
                            </div>
                            <div className="grid grid-cols-3 gap-3">
                                <div className="bg-white border border-soft-200 rounded-xl px-4 py-2 shadow-sm">
                                    ...
                                    <p className="text-sm font-bold text-navy-500">{user.turma || '-'}</p>
                                </div>
                                <div className="bg-white border border-soft-200 rounded-xl px-4 py-2 shadow-sm">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Turno</p>
                                    <p className="text-sm font-bold text-navy-500">{user.turno || '-'}</p>
                                </div>
                                <div className="bg-white border border-soft-200 rounded-xl px-4 py-2 shadow-sm">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Status</p>
                                    <p className="text-sm font-bold text-success-600">Regular</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {responsavel && (
                        <div className="flex items-center gap-8 bg-soft-50 p-6 rounded-3xl border border-soft-200">
                            <img src={responsavel.foto_url} alt={responsavel.nome} className="w-32 h-32 rounded-full shadow-md border-4 border-white flex-shrink-0" />
                            <div className="flex-1 space-y-3">
                                <div className="bg-white border border-soft-200 rounded-xl px-5 py-3 shadow-sm">
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Nome do Responsável</p>
                                    <h2 className="text-2xl font-black text-navy-500">{responsavel.nome}</h2>
                                </div>
                                <div className="grid grid-cols-3 gap-3">
                                    <div className="bg-white border border-soft-200 rounded-xl px-4 py-2 shadow-sm">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Parentesco</p>
                                        <p className="text-sm font-bold text-navy-500">{responsavel.parentesco || 'Responsável'}</p>
                                    </div>
                                    <div className="bg-white border border-soft-200 rounded-xl px-4 py-2 shadow-sm col-span-2">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Contato</p>
                                        <p className="text-sm font-bold text-navy-500">{responsavel.telefone || '-'}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {/* FOOTER BUTTONS */}
                <div className="grid grid-cols-2 gap-4 p-6 bg-slate-50 border-t border-soft-200">
                    <button
                        onClick={onCancel}
