...
function UserListPanel({ onClose, onShowToast }) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(t('usuarios.erro.carregar'));
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl font-bold text-navy-500">{t('usuarios.titulo')}</h2>
          <p className="text-xs text-slate-400 mt-0.5">{t('usuarios.subtitulo')}</p>
        </div>
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-sm text-slate-400">{t('usuarios.carregando')}</span>
        </div>
      ) : (
          ...
          <table className="w-full relative">
            <thead className="sticky top-0 bg-soft-50 z-10 shadow-sm">
              <tr className="text-left">
                <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('comum.id')}</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('comum.nome')}</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('comum.tipo')}</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('comum.turma')}</th>
                <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider">{t('comum.status')}</th>
              ...
              {filteredUsers.length === 0 ? (
                ...
                <tr key={u.id} className="hover:bg-soft-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm">
                    {u.ativo !== false
                      ? <span className="inline-flex items-center gap-1.5 text-success-600 font-semibold text-xs"><span className="w-1.5 h-1.5 rounded-full bg-success-500"></span>{t('comum.ativo')}</span>
                      : <span className="inline-flex items-center gap-1.5 text-danger-600 font-semibold text-xs"><span className="w-1.5 h-1.5 rounded-full bg-danger-500"></span>{t('comum.inativo')}</span>}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-3">
                      ...
                      >
                        <LucideIcon name="edit-2" size={14} /> {t('acao.editar')}
                      </button>
                      {u.ativo !== false && u.tipo !== 'RESPONSAVEL' && (
                        ...
                        >
                          <LucideIcon name="user-x" size={14} /> {t('acao.desativar')}
                        </button>
                      )}
                    </div>
                  </td>
      ...
      <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl overflow-hidden animate-zoom-in">
        ...
        <div className="bg-navy-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">{t('usuarios.editar.titulo')}</h2>
              <p className="text-xs text-white/50">{t('usuarios.editar.subtitulo')} {user?.id}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1">{t('usuarios.nome.completo')}</label>
            <input
              type="text"
              value={form.nome}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-1">{t('usuarios.turma.se.aluno')}</label>
              <input
                type="text"
                value={form.turma || ''}
                onChange={(e) => setForm({ ...form, turma: e.target.value })}
                disabled={form.tipo !== 'ALUNO'}
                placeholder={t('usuarios.turma.exemplo')}
          ...
          {form.tipo === 'RESPONSAVEL' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">{t('usuarios.telefone')}</label>
                <input
                  type="text"
                  value={form.telefone || ''}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">{t('comum.parentesco')}</label>
                <input
                  type="text"
                  value={form.parentesco || ''}
                  onChange={(e) => setForm({ ...form, parentesco: e.target.value })}
                  className="w-full bg-soft-50 border border-soft-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-accent-500 text-navy-500"
                  placeholder={t('usuarios.parentesco.exemplo')}
                />
              </div>
            </div>
          )}
